...
parser.add_argument('--data_folder', default='/content/GRU4REC-pytorch/data', type=str)
parser.add_argument('--train_data', default='/content/GRU4REC-pytorch/data/preprocessed_datarecSys15TrainOnly.csv', type=str)
parser.add_argument('--valid_data', default='/content/GRU4REC-pytorch/data/preprocessed_datarecSys15Valid.csv', type=str)
parser.add_argument("--is_eval", action='store_true') #should be used during testing and eliminated during training
parser.add_argument('--load_model', default=None,  type=str)
parser.add_argument('--checkpoint_dir', type=str, default='/content/GRU4REC-pytorch/checkpoint')
...
def init_model(model):
    if args.sigma is not None:
        for p in model.parameters():#P ==> MODEL PARAMETERS 
            if args.sigma != -1 and args.sigma != -2: 
                sigma = args.sigma
                p.data.uniform_(-sigma, sigma) # WITH REPLACEMENT(DEL OLD PARAM AND INTIALIZE WITH THIS)  
...
def main():
    ...
    if not args.is_eval: #training
        ...
        if args.load_model is not None:
            ...
            loss , recall , precision , mrr = evaluation.eval(valid_data, batch_size)
            print("Final result: recall = {:.2f},precision = {:.2f},mrr = {:.2f}".format(recall,precision,mrr))
        else:
