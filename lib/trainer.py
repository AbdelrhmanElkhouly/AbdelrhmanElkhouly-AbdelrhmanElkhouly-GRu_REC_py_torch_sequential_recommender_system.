...
class Trainer(object):
    ...
    def train(self, start_epoch, end_epoch, start_time=None):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            loss, recall,precision, mrr = self.evaluation.eval(self.eval_data, self.batch_size)
    ...
    def train_epoch(self, epoch):
        ...
        for ii, (input, target, mask) in tqdm(enumerate(dataloader), total=len(dataloader.dataset.df) // dataloader.batch_size, miniters = 1000):
            ...
            # output sampling
            logit_sampled = logit[:, target.view(-1)]#put target with scores
            
            loss = self.loss_func(logit_sampled)
            losses.append(loss.item())
            loss.backward()#calac error on each layer backp  
